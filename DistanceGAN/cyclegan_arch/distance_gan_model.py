...
Description: Description: Code for fooling Steering angle predictors using DistanceGAN. 
                Base code taken from Benaim et al and rest of the parameters and
...
S_angle = steering_model_loader.autopilot_model_loader().cuda(1) # needs -1 to +1
# S_angle = steering_model_loader.comma_ai_model_loader().cuda() # needs -1 to +1
...
class DistanceGANModel(CycleGANModel):
    ...
    def backward_G(self):
        ...
        if self.opt.use_cycle_loss:
            ...
            if self.opt.identity > 0:
                ...
                self.loss_idt_A = self.criterionIdt(self.idt_A, self.real_B) * self.opt.lambda_B * lambda_idt
                # G_B should be identity if real_A is fed.
                self.idt_B = self.netG_B.forward(self.real_A)
                self.loss_idt_B = self.criterionIdt(self.idt_B, self.real_A) * self.opt.lambda_A * lambda_idt
            else:
                self.loss_idt_A = 0
                self.loss_idt_B = 0
            ...
            self.loss_F_regress = theta - self.L1loss(self.fake_B_angle, self.real_A_angle)

            self.loss_G += self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B
            self.loss_G = (1-alpha)*self.loss_G + alpha*(self.loss_F_regress)
    ...
    def get_current_errors(self):
        if self.opt.use_cycle_loss:
            ...
            F_regress = self.loss_F_regress.data[0]
            idt_A = self.loss_idt_A.data[0]
            idt_B = self.loss_idt_B.data[0]
            return CycleGANModel.get_current_errors(self, Dist_A, Dist_B, F_regress, idt_A, idt_B)
